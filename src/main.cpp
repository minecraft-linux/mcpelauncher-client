...
int main(int argc, char* argv[]) {
    ...
    argparser::arg<int> windowHeight(p, "--height", "-wh", "Window height",
                                     480);
    argparser::arg<bool> disableFmod(
        p, "--disable-fmod", "-df", "Disables usage of the FMod audio library");
    ...
    for(auto&& entry : libC) {
        hooks.emplace_back(
            mcpelauncher_hook_t{entry.first.data(), entry.second});
    }
    ...
    try {
        ...
        Log::error("LAUNCHER",
                   "Could not find the game, use the -dg flag to fix this "
    ...
    if(options.graphicsApi == GraphicsApi::OPENGL_ES2) {
        ...
        // Otherwise linker has to hide the symbols from dlsym in
        // libminecraftpe.so
    ...
    if(!handle) {
        Log::error("Launcher",
                   "Failed to load Minecraft library, please reinstall or wait "
                   "for an update to support the new release");
        return 51;
    }
    ...
    if(linker::dlsym(handle, "bgfx_init")) {
        // The directinput mode is incompatible with the most of renderdragon
        // enabled games Hide the availability of these symbols, until the bug
        // is fixed
    ...
    if(options.graphicsApi == GraphicsApi::OPENGL) {
        try {
            ...
            Log::error("GLCOREPATCH", "Failed to apply glcorepatch: %s",
                       ex.what());
            options.graphicsApi = GraphicsApi::OPENGL_ES2;
        }
    }
