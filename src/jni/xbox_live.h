...
class XboxInterop : public FakeJni::JObject {
    ...
    static std::shared_ptr<FakeJni::JString> getLocalStoragePath(
        std::shared_ptr<Context> context);

    static std::shared_ptr<FakeJni::JString> readConfigFile(
        std::shared_ptr<Context> context);
    ...
    static void ticketCallback(FakeJni::Jvm const &vm,
                               std::string const &ticket, int requestCode,
                               int errorCode, std::string const &errorStr);
    ...
    static void invokeEventInitialization(
        FakeJni::Jvm const &vm, FakeJni::JLong userPtr,
        std::string const &ticket, std::shared_ptr<XboxLoginCallback> callback);
};
...
class XboxLocalStorage : public FakeJni::JObject {
    ...
    static std::shared_ptr<FakeJni::JString> getPath(
        std::shared_ptr<Context> context);
};
