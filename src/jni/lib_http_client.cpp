...
FakeJni::JBoolean HttpClientRequest::isNetworkAvailable(
...
void HttpClientRequest::setHttpMethodAndBody(
    ...
    if(conttype.length()) {
        header =
            curl_slist_append(header, ("Content-Type: " + conttype).c_str());
...
void HttpClientRequest::doRequestAsync(FakeJni::JLong sourceCall) {
    ...
    if(ret == CURLE_OK) {
        auto method =
            getClass().getMethod("(JLcom/xbox/httpclient/HttpClientResponse;)V",
                                 "OnRequestCompleted");
...
std::shared_ptr<FakeJni::JString> HttpClientResponse::getHeaderNameAtIndex(
    FakeJni::JInt index) {
    return std::make_shared<FakeJni::JString>(headers[index].name);
}

std::shared_ptr<FakeJni::JString> HttpClientResponse::getHeaderValueAtIndex(
    FakeJni::JInt index) {
...
void NativeOutputStream::WriteAll(std::shared_ptr<FakeJni::JByteArray> data) {
    ...
    method->invoke(frame.getJniEnv(), this, call_handle,
                   frame.getJniEnv().createLocalReference(data),
                   (FakeJni::JInt)0, (FakeJni::JInt)data->getSize());
}
