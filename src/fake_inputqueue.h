...
struct FakeMotionEvent : FakeInputEvent {
    ...
    int32_t pointerId;
    float x, y;
    std::function<void (int32_t axis)> axisFunction;
    ...
    FakeMotionEvent(int32_t source, int32_t deviceId, int32_t action, int32_t pointerId, float x, float y, std::function<void (int32_t axis)> axisFunction) : FakeInputEvent(source, AINPUT_EVENT_TYPE_MOTION, deviceId), action(action), pointerId(pointerId), x(x), y(y), axisFunction(std::move(axisFunction)) {}
};
